...
function collectVitals(a){if(!t.PerformanceObserver)return;var w=websiteId(a);if(!w)return;try{var o=r();o.website=w;var m={},sent=0;function flush(){if(sent)return;try{var nav=t.performance.getEntriesByType('navigation');if(nav.length&&!m.ttfb)m.ttfb=Math.round(nav[0].responseStart)}catch(_){}if(!(m.lcp||m.inp||m.cls||m.fcp||m.ttfb))return;sent=1;o.lcp=m.lcp;o.inp=m.inp;o.cls=m.cls;o.fcp=m.fcp;o.ttfb=m.ttfb;send('performance',o)}function obs(type,key,fn){try{new PerformanceObserver(function(list){var x=list.getEntries();if(x.length)m[key]=fn(x[x.length-1])}).observe({type:type,buffered:true})}catch(_){}}obs('largest-contentful-paint','lcp',function(x){return Math.round(x.startTime)});obs('first-contentful-paint','fcp',function(x){return Math.round(x.startTime)});try{new PerformanceObserver(function(list){var x=list.getEntries();if(x.length)m.inp=Math.round(x[x.length-1].duration)}).observe({type:'event',buffered:true,durationThreshold:40})}catch(_){}try{var cls=0;new PerformanceObserver(function(list){var x=list.getEntries(),i,e;for(i=0;i<x.length;i++){e=x[i];if(!e.hadRecentInput)cls+=e.value}m.cls=Math.round(cls*1e4)/1e4}).observe({type:'layout-shift',buffered:true})}catch(_){}obs('navigation','ttfb',function(x){return Math.round(x.responseStart)});setTimeout(flush,1e4);function onLeave(){flush()}d.addEventListener('visibilitychange',function(){if(d.visibilityState==='hidden')onLeave()});t.addEventListener('pagehide',onLeave)}catch(_){}}
