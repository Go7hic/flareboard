...
function collectVitals(a){if(vitalsSent||!t.PerformanceObserver)return;var w=websiteId(a);if(!w)return;vitalsSent=true;try{var o=r();o.website=w;var m={};function obs(type,key,fn){try{new PerformanceObserver(function(list){var x=list.getEntries();if(x.length)m[key]=fn(x[x.length-1])}).observe({type:type,buffered:true})}catch(_){}}obs('largest-contentful-paint','lcp',function(x){return Math.round(x.startTime)});obs('first-contentful-paint','fcp',function(x){return Math.round(x.startTime)});try{new PerformanceObserver(function(list){var x=list.getEntries();if(x.length)m.inp=Math.round(x[x.length-1].duration)}).observe({type:'event',buffered:true,durationThreshold:40})}catch(_){}obs('layout-shift','cls',function(x){return x.value});obs('navigation','ttfb',function(x){return Math.round(x.responseStart)});setTimeout(function(){if(m.lcp||m.inp||m.cls||m.fcp||m.ttfb){o.lcp=m.lcp;o.inp=m.inp;o.cls=m.cls;o.fcp=m.fcp;o.ttfb=m.ttfb;send('performance',o)}},3000)}catch(_){}}
